...
html = headTitle({Text: 'Вход в '});
document.getElementsByClassName('auth__block')[0].innerHTML += html;

// подпись
const descp = Handlebars.templates.descp;
html = descp({Text: 'Planexa — это визуальный инструмент, который позволяет вашей команде управлять проектами, рабочими процессами и заданиями'});
document.getElementsByClassName('auth__block')[0].innerHTML += html;
...
document.getElementsByClassName('auth__block_dont')[0].innerHTML += 'Нет аккаунта в Planexa?'

const link = document.createElement('a');
link.href = 'signup.html';
link.className = 'auth__block_link';
document.getElementsByClassName('auth__block_signup')[0].appendChild(link);
document.getElementsByClassName('auth__block_link')[0].innerHTML += 'Зарегистрироваться';
